    ...
    : QMainWindow{parent}
{
    ...
    connect(m_leftSeries, &QLineSeries::pointAdded, &m_leftScaler, [=](int index) {
        scaleLineSeriesAxis(m_leftSeries, axisYLeft, &m_leftScaler, index);
    });

    connect(m_rightSeries, &QLineSeries::pointAdded, &m_rightScaler, [=](int index) {
        scaleLineSeriesAxis(m_rightSeries, axisYRight, &m_rightScaler, index);
    });
...
AxisSetter *ChartWindow::createAxis(QString titleText)
{
...
void ChartWindow::scaleLineSeriesAxis(QLineSeries *lineSeries, AxisSetter *axisY, AxisScaler *scaler, int index)
{
...
void ChartWindow::scaleLineSeries(QLineSeries *series, double scale, int index)
{
    if(series) {
